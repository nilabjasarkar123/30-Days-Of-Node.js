...
// Product Statistics Aggregation Pipeline
/**
...
async function getProductStatistics() {
  try {
    ...
    const aggregatedResults = await Product.aggregate(pipeline);
    return aggregatedResults[0]; // Returns the aggregated product statistics
  } catch (error) {
    throw new Error(`Error calculating product statistics: ${error.message}`);
  }
}
...
app.listen(PORT, async () => {
  ...
  // Display aggregated product statistics
  const productStats = await getProductStatistics();
  console.log("Aggregated Product Statistics:", productStats);
});
